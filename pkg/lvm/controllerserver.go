...
import (
	...
	"github.com/kubernetes-csi/csi-driver-lvm/pkg/lvmclient"
...
func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if req.GetCapacityRange().GetLimitBytes() > 0 {
		capacity = int64(req.GetCapacityRange().GetLimitBytes())
	} else {
	...
	*/
	...
	if exVol, err := getVolumeByName(req.GetName()); err == nil {
		...
		fmt.Println("same name volume been creaeded")
	...
	//nodechoice := ChoseNodePolicy(req.GetAccessibilityRequirements(), capacity)

	//response := &csi.CreateVolumeResponse{}
	volumeID := uuid.NewUUID().String()

	//**********************************
	/*	parms := req.GetParameters()
		vgName := ""
	...
	*/
	...
	if req.GetAccessibilityRequirements() != nil && req.GetAccessibilityRequirements().Requisite != nil {
		response = &csi.CreateVolumeResponse{
			Volume: &csi.Volume{
				VolumeId:           volumeID,
				...
				VolumeContext:      req.GetParameters(),
				ContentSource:      req.GetVolumeContentSource(),
				AccessibleTopology: req.GetAccessibilityRequirements().Requisite,
			},
		}
	} else {
		response = &csi.CreateVolumeResponse{
	...
	fmt.Println("end create volume")
	return response, nil

}

func (cs *controllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	fmt.Printf("begin delete volume %s \n", req.GetVolumeId())
	// Check arguments
	...
	if err := lvmclient.DeleteVolInLVM(ctx, volId); err != nil {
...
func (cs *controllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	*/
	...
	vgName, fstype, err := util.GetVgnameForVolume(ctx, volumeID)
	...
	if _, ok := parms[SecretNameTag]; ok {
		secretnm = parms[SecretNameTag]
	}
	if secretnm == "" {
		return nil, status.Error(codes.Internal, "error with input secretnm is empty")
	}

	secretnamespace := ""

	if _, ok := parms[SecretNamespaceTag]; ok {
		secretnamespace = parms[SecretNamespaceTag]
	}

	if secretnamespace == "" {
		return nil, status.Error(codes.Internal, "error with input secretnamespace is empty")
	}

	s3endpoint := ""

	if _, ok := parms[S3EndPointTag]; ok {
		s3endpoint = parms[S3EndPointTag]
	}

	if s3endpoint == "" {
		return nil, status.Error(codes.Internal, "error with input s3endpoint is empty")
	}

	s3env, err := util.GetS3ENV(ctx, secretnm, secretnamespace, s3endpoint)
	if err != nil {
		return nil, status.Error(codes.Internal, "faild  to get s3 env")
	}

	timeout := "1h"
	if _, ok := parms["timeout"]; ok {
	...
	//size := fmt.Sprintf("%dM", hostPathVolume.VolSize/mib)
	fmt.Printf("volume id is %s for snap \n", volumeID)
	fmt.Printf("snap id is %s for snap \n", snapshotID)

	err = lvmclient.StrartSnap(ctx, volumeID, vgName, "1G", snapshotID, s3env, fstype,timeout)

	if err != nil {
		return nil, status.Error(codes.Internal, "error with startsnap")
	}
...
func (cs *controllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	...
	err := util.DeleteSnap(ctx, snapshotID)
	if err != nil {
		return nil, err
	}
